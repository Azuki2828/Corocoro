...
float4 PSMain(PSInput In) : SV_Target0
{
	...
	float3 albedoColor = albedoAndShadowReceiverTexture.Sample(g_sampler, In.uv + uOffset * modelOption[ligID].uvNoiseMul).xyz;
	albedoColor = lerp(
		albedoColor,
		float3(albedoColor.y, albedoColor.y, albedoColor.y) * 0.5f,
		modelOption[ligID].uvNoiseMul
	);
	int shadowReceiverFlg = albedoAndShadowReceiverTexture.Sample(g_sampler, In.uv).w;
